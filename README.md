...
| 로컬 Ollama vs Gemini API(6문항) | 응답시간 / 평균 자기판정 점수 | 3.97s vs 38.1s(9.6배) / 85.8 vs 100.0(과대평가 의심) |

## 확장 시도 (선택 엔진 비교·검색 고도화 등)

Main Quest 3의 "조금 더 발전시키고 싶다면?" 항목에 대응해 실제로 시도한 것들.
전부 실측 근거와 함께 [docs/EXPERIMENTS.md](./docs/EXPERIMENTS.md)에 기록되어 있다.

| 시도 | 결과 | 상세 |
|---|---|---|
| **선택 엔진 비교**(로컬 Ollama ↔ Gemini API) | 완료 — Gemini가 9.6배 느리고, 자기 판정이 난이도 무관하게 전부 100점(변별력 의심)이라 기본 엔진은 Ollama 유지. 화면에서 두 엔진을 직접 골라 쓸 수 있다 | 사이클 8 |
| **검색 고도화**(임베딩 모델 교체) | 완료 — e5-small(384d)→EmbeddingGemma(768d)로 바꿔 hitRate 0.91→1.00, 오차단 3건→0건 | 사이클 2 |
| **검색 고도화**(쿼리 확장) | 시도했으나 **채택 안 함** — 구어체 질문 12개로 실측했더니 hitRate가 오히려 0.67→0.56으로 떨어짐. 결과가 기대와 달라도 숨기지 않고 원인과 함께 기록 | 사이클 6 |
| **UI 완성도**(검색 진행 상태 표시) | 완료 — 질문~답변 사이 빈 화면을 "검색 중...→생성 중..." 표시로 채움 | app.js |
| **UI 완성도**(모바일 반응형) | 확인 완료 — 375px 뷰포트에서 가로 스크롤·레이아웃 깨짐 없음 | — |
| **UI 완성도**(출처 칩 정확도) | 완료 — 검색 후보 15개를 다 보여주던 것을, 답변이 실제로 인용한 것만 보이게 수정 | 사이클 7 인접 커밋 |
| **피드백 축적→개선 사이클** | 메커니즘만 구현(👍👎 → localStorage) — 실제 축적·재학습은 배포 후 실사용자가 쌓여야 의미 있어 지금은 절반만 해당 | app.js |
